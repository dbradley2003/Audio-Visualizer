...
struct ParticleGenerator {
  ...
  void Init() {
    ...
    for (int i{}; i < PARTICLE_COUNT; ++i) {
      ...
      p.pos = {(float)GetRandomValue(0, width),
  ...
  void update(float rawBass, float rawTreble) {
    ...
    for (auto &p : particles) {
      ...
      if (p.pos.x < -border)
        p.pos.x = widthLimit;
      else if (p.pos.x > widthLimit)
        p.pos.x = -border;

      if (p.pos.y < -border)
        p.pos.y = heightLimit;
      else if (p.pos.y > heightLimit)
        p.pos.y = -border;
      ...
      if (energy < 0.5f)
        p.baseColor = ColorLerp(P_DEEP_VOID, P_NEON_CYAN, energy * 2.0f);
      else
        p.baseColor =
            ColorLerp(P_NEON_CYAN, P_HYPER_HOT, (energy - 0.5f) * 2.0f);
  ...
  void Draw() const {
    ...
    for (const auto &p : particles) {
      if (p.alphaOffset <= 0.01f)
        continue;

      float coreSize = p.pulseSize * 0.3f;

      Vector2 corePos = {p.pos.x + p.pulseSize * 0.25f,
                         p.pos.y + p.pulseSize * 0.25f};


      DrawCircleGradient(corePos.x, corePos.y, coreSize * 1.1,
      ColorAlpha(p.baseColor, p.alphaOffset), 	ColorAlpha(BLACK, 0.6f));

      DrawRectangleV(corePos, {coreSize, coreSize},
                     ColorAlpha(p.baseColor, p.alphaOffset));
    }
    EndBlendMode();
  }
